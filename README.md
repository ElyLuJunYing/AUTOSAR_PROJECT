# AUTOSAR 车窗控制系统项目与学习笔记

## 项目介绍：智能车窗控制系统（应用层软件开发 · 2025.03–2025.05）

**项目描述**：基于 AUTOSAR CP 的智能车窗控制系统应用层软件开发，针对车窗使能、自动升降和手动升降的需求，完成从需求分析、底层通信配置、应用层控制逻辑建模到测试验证的全流程，确保多模式控制功能正确稳定。

**技术栈**：Simulink、Stateflow、DBC、CAN、COM、PduR、CanIf、Can Driver、MIL、TSMaster、AUTOSAR CP

### 核心工作

1. **CAN 通信协议栈**：从 DBC 收发报文/信号设计出发，完成 COM / PduR / CanIf / Can 全链路 CFG 配置（I-Pdu、发送模式、L-Pdu、CAN 控制器、波特率等），并打通 SWC-Port 与 Com-Signal 映射 —— 建立应用层与底层通信的桥梁。
2. **车窗控制逻辑建模**：基于 10ms 周期 Runnable + Stateflow 三状态机，通过中间状态、超时转移、边沿触发，实现使能延迟关闭、遥控长按自动升降、手动长按扩展控制等多种模式。
